...
export default function AdminCategoriesPage() {
  ...
  const onSubmit = async (values: CategoryFormValues) => {
    ...
    try {
      ...
      if (modalMode === "edit" && activeCategory) {
        await api.put(`/Categories/${activeCategory.id}`, payload);
        toast.success(`Category "${values.name}" updated successfully!`);
        setCategories((prev) =>
          prev.map((c) => (c.id === activeCategory.id ? { ...c, ...payload } : c))
        );
      } else {
  ...
  const columns: Column<Category>[] = [
    ...
    },
    {
      header: "ID",
      accessorKey: "id",
      cell: (cat) => <span className="text-stone-400 text-xs font-mono">#{cat.id}</span>,
      className: "w-20",
    },
    {
      header: "Actions",
